...
class Message(BaseModel):
    ...
    def _string_serializer(self) -> dict:
        """Serialize message content and add tool call keys if applicable."""
        # Preallocate content string building for faster performance
        content_list = []
    ...
    def _add_tool_call_keys(self, message_dict: dict) -> dict:
        ...
        """
        # Using list comprehension directly in message_dict assignment for efficiency
        if self.tool_calls:
        ...
        # Immediately check tool_call_id and name for message_dict assignment
        if self.tool_call_id:
            assert self.name, "name is required when tool_call_id is not None"
            message_dict.update({"tool_call_id": self.tool_call_id, "name": self.name})
