...
def has_execeptions(tree, num_lines):
    def traverse(node):
        if node.type == "try_statement":
            return True  # Early exit if any 'try_statement' is found
        for child in node.children:
            if traverse(child):  # Only continue if no exception found in child
                return True
        return False

    # Traverse the tree and set the result based on the presence of 'try_statement'
    has_exceptions = traverse(tree.root_node)
    return {"total_has_exceptions": 1 if has_exceptions else 0}
