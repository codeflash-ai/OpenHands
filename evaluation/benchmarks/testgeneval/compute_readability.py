...
def string_stats(tree, num_lines):
    ...
    # Function to traverse the AST and collect string literal lengths
    def traverse(node):
        if node.type == "string_literal":
            # Adding the length of the string literal directly
            literal_length = (
                len(node.text.decode("utf8")) - 2
            )  # Exclude quotation marks
            string_literal_lengths.append(literal_length)
        for child in node.children:
            traverse(child)

    traverse(tree.root_node)

    # Calculate the average string length using precomputed lengths
    total_length = sum(string_literal_lengths)
    avg_length = total_length / num_lines if num_lines else 0  # Avoid division by zero
