...
def distinct_methods_stats(tree, num_lines):
    ...
    def traverse(node):
        nonlocal method_declaration_count
        if node.type == "method_declaration":
            method_declaration_count += 1
            for child in node.children:
                if child.type == "identifier":
                    # Only add the method name once.
                    method_names.add(child.text.decode("utf8"))
                    break
        else:
            # Traverse only when node is not a method_declaration
            for child in node.children:
                traverse(child)
    ...
    # Simplify the ratio calculation logic to avoid additional checks
    total_method_ratio = (
        total_distinct_methods / (method_declaration_count - total_distinct_methods)
        if method_declaration_count > total_distinct_methods
        else 0
    )
