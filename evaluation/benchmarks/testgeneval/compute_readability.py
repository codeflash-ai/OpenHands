...
def identifier_stats(tree, num_lines):
    ...
    def count(node):
        ...
        if node.type == "identifier":
            ...
            local_counts[identifier] = local_counts.get(identifier, 0) + 1
            iden_length = len(identifier)
            if iden_length > local_max_length:
                local_max_length = iden_length

        for child in node.children:
            child_counts, child_max_length = count(child)
            for iden, count in child_counts.items():
                local_counts[iden] = local_counts.get(iden, 0) + count
            if child_max_length > local_max_length:
                local_max_length = child_max_length

        return local_counts, local_max_length

    identifier_counts, max_identifier_length = count(root_node)
