...
def string_stats(tree, num_lines):
    # Initialize total length for calculating the average
    total_length = 0

    # Function to traverse the AST and collect string literals
    def traverse_and_collect_length(node):
        nonlocal total_length
        if node.type == "string_literal":
            # Directly accumulate the length of the string literal,
            # excluding the quotation marks
            total_length += len(node.text.decode("utf8")) - 2  # account for the quotes
        for child in node.children:
            traverse_and_collect_length(child)

    # Perform traversal and length aggregation
    traverse_and_collect_length(tree.root_node)

    # Calculate and return the average string length
