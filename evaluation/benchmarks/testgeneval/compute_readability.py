...
def average_nulls_stats(tree, num_lines):
    total_nulls = 0
    max_nulls_on_any_line = 0
    nulls_per_line = {}  # Dictionary to count nulls per line

    # Flatten the recursive traverse calls into a loop using a stack for performance improvement
    ...
    while stack:
        ...
        if node.type == "null_literal":
            ...
            nulls_per_line[line_number] = nulls_per_line.get(line_number, 0) + 1
            # Update max nulls on any line during traversal
            if nulls_per_line[line_number] > max_nulls_on_any_line:
                max_nulls_on_any_line = nulls_per_line[line_number]

        # Extend stack with children instead of making recursive calls
