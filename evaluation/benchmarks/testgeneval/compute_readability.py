...
def numbers_floats_stats(tree, num_lines):
    # This function calculates the total number of numeric literals and the
    # total number of floats within a given tree.

    def traverse(node):
        # Initialize counts for numbers and floats
        total_numbers, total_floats = 0, 0

        # Stack based approach to avoid recursion overhead.
        stack = [node]
        while stack:
            current_node = stack.pop()

            # Check node type to determine if it's a numeric literal
            if current_node.type in ["integer_literal", "decimal_literal"]:
                total_numbers += 1
                node_text = current_node.text.decode("utf8")

                # Check if the number is a float
                if "." in node_text or "e" in node_text.lower():
                    total_floats += 1

            # Append children to the stack for further traversal
            stack.extend(current_node.children)

        return total_numbers, total_floats

    # Traverse the tree starting from the root and retrieve counts
    total_numbers, total_floats = traverse(tree.root_node)
