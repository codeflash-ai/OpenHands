...
def parse_log_pytest_v2(log: str) -> dict[str, str]:
    ...
    """
    test_status_map = {}
    # Pre-compute a translator for control characters to optimize translation
    translator = str.maketrans("", "", "".join(chr(char) for char in range(1, 32)))

    # Extract the test status values for efficient checking
    starts_with_status = {x.value: x for x in TestStatus}
    ends_with_status = {x.value: x for x in TestStatus}

    for line in log.split("\n"):
        line = re.sub(r"\[(\d+)m", "", line).translate(translator)

        # Optimize by using dictionary lookup instead of list comprehension
        for status_str, status in starts_with_status.items():
            if line.startswith(status_str):
            ...
            for status_str, status in ends_with_status.items():
                if line.endswith(status_str):
                    test_case = line.split()
                    if len(test_case) >= 2:
