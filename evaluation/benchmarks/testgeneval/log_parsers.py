...
def parse_log_pytest_options(log: str) -> dict[str, str]:
    ...
    test_status_map = {}
    test_status_values = {x.value: x for x in TestStatus}

    for line in (
        log.splitlines()
    ):  # Optimized: Using splitlines(), which is faster for large logs
        line_prefix = line.split(" ", 1)[0]
        test_status = test_status_values.get(line_prefix)
        ...
        if test_status:
            ...
            if has_option:
                ...
                # Simplified option checking logic
                if option.startswith("/") and not (
                    option.startswith("//") or "*" in option
                ):
