...
def parse_log_seaborn(log: str) -> dict[str, str]:
    ...
    Returns:
        dict: test case to test status mapping
    """
    # Using a generator for split lines to avoid generating a large intermediate list
    test_status_map = {}
    passed_value = TestStatus.PASSED.value
    failed_value = TestStatus.FAILED.value

    for line in log.split("\n"):
        if line.startswith(failed_value):
            test_status_map[line.split()[1]] = failed_value
        elif line.startswith(passed_value):
            test_status_map[line.split()[1]] = passed_value
        elif f" {passed_value} " in line:
            parts = line.split()
            if parts[1] == passed_value:
                test_status_map[parts[0]] = passed_value
    return test_status_map
