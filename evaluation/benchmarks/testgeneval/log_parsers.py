...
def parse_log_sympy(log: str) -> dict[str, str]:
    ...
    # Optimize by pre-creating status suffixes for quick checks
    fail_suffix = "[FAIL]"
    ok_suffix = "[OK]"
    error_suffix = " E"
    failed_suffix = " F"
    ...
    # Direct update of the test_status_map for matched patterns
    for match in matches:
        test_case = f"{match[1]}.py:{match[2]}"
        test_status_map[test_case] = TestStatus.FAILED.value

    # Use a single loop to check all lines instead of splitting twice
    for line in log.splitlines():
        stripped_line = line.strip()

        # Direct indexing instead of expensive string operations when possible
        if stripped_line.startswith("test_"):
            # Check specific status conditions
            test = stripped_line.split()[0]
            if stripped_line.endswith(error_suffix):
                test_status_map[test] = TestStatus.ERROR.value
            elif stripped_line.endswith(failed_suffix):
                test_status_map[test] = TestStatus.FAILED.value
            elif stripped_line.endswith(passed_suffix):
                test_status_map[test] = TestStatus.PASSED.value
