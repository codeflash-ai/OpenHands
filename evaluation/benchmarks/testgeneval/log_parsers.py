...
def parse_log_matplotlib(log: str) -> dict[str, str]:
    ...
    """
    test_status_map = {}
    test_status_values = {x.value for x in TestStatus}  # Gather test status values once
    for line in log.split("\n"):
        if "MouseButton" in line:
            line = line.replace("MouseButton.LEFT", "1").replace(
                "MouseButton.RIGHT", "3"
            )

        # Use a set lookup for faster evaluation if line starts with any valid status
        if any(line.startswith(value) for value in test_status_values):
            # Additional parsing for FAILED status
            if line.startswith(TestStatus.FAILED.value):
                line = line.replace(" - ", " ")
